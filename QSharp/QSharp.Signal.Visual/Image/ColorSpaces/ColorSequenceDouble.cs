...
{
    public class ColorSequenceDouble : IColorAccessor<double>, IColorSequence<double>
    {
        public double this[int offset]
        {
            get { return _list[offset]; }
            set { _list[offset] = value; }
        }

        public ColorSequenceDouble(IList<double> list, int stride, params int[] componentOrder)
        {
            _list = list;
            _stride = stride;
